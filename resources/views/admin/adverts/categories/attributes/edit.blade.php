...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.advert.categories.attributes.update', [$category, $attribute]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <textarea  placeholder="write your options separated by commas" id="variants" type="text" class="form-control{{ $errors->has('sort') ? ' is-invalid' : '' }}" name="variants">{{ old('variants', implode("\n", $attribute->variants)) }}</textarea>
